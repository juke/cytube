 ...
 */
if (!playlistEnhancer) {
    var playlistEnhancer = {}
}
if (window[CHANNEL.name] && window[CHANNEL.name].modulesOptions && window[CHANNEL.name].modulesOptions.playlistEnhancer) {
    playlistEnhancer.recentMedia = window[CHANNEL.name].modulesOptions.playlistEnhancer.recentMedia
} else {
    playlistEnhancer.recentMedia = true
}
/*!
 **|  Playlist Helper
 **@preserve
...
function playlist(active) {
    var __selector = "#queue > .queue_entry";
    var _playlist = [];
    if (active) {
        __selector += ".queue_active"
    }
    ...
    return active && _playlist[0] || _playlist
}

function plsort_export() {
    return sorted_list = playlist().sort(((a, b) => {
        return a.media.title < b.media.title ? -1 : a.media.title > b.media.title ? 1 : 0
    })).map((cv => {
        return {
            name: cv.media.title,
            link: cv.media.type + ":" + cv.media.id
        }
    })).map((cv => {
...
function generateThumbnailPopover(target) {
    var css = "playlist-thumbnail";
    var type = target.data().media.type;
    var id = target.data().media.id;
    var DOM = '<img src="__url" class="__class">'.replace(/__class/, css);

    function applyPopover(thumb) {
        target.popover({
            html: true,
            placement: function() {
    ...
    function genThumb(url) {
        var thumb = DOM.replace(/__url/, url);
        return applyPopover(thumb)
    }
    if (type == "yt") {
        var url = "http://img.youtube.com/vi/__id/0.jpg".replace(/__id/, id);
        genThumb(url);
        return target.addClass("thumbed")
    }
    if (type == "vi") {
        var api = "http://vimeo.com/api/v2/video/__id.json".replace(/__id/, id);
    ...
    if (type == "dm") {
        var url = "http://www.dailymotion.com/thumbnail/video/__id".replace(/__id/, id);
        genThumb(url);
        return target.addClass("thumbed")
    }
    if (type == "gd") {
        var url = "http://thumbs.pink.horse/drive/__id".replace(/__id/, id);
        genThumb(url);
        return target.addClass("thumbed")
    }
    if (type == "gp") {
        var url = "http://thumbs.pink.horse/picasa/__id".replace(/__id/, id);
        genThumb(url);
        return target.addClass("thumbed")
    }
}

function playlist_scan() {
    $("#queue > .queue_entry:not(.thumbed)").each(function() {
        generateThumbnailPopover($(this))
    })
}

function trimPopoverOrphans() {
    return $("#queue .popover").remove()
}
if (!CLIENT.thumbnail_scanner) {
    CLIENT.thumbnail_scanner = true;
    $("head").append($("<style>").prop("id", "thumbnailStyle").text(".playlist-thumbnail { max-height: 180px; max-width: 240px; border-radius: 4px; }"));
    playlist_scan();
    socket.on("queue", playlist_scan);
    socket.on("playlist", playlist_scan);
    socket.on("delete", function() {
        trimPopoverOrphans()
    });
    $("#queue").on("mouseleave", trimPopoverOrphans)
}(function() {
    if (CLIENT.playlistInline) {
        return
    }
    CLIENT.playlistInline = true;
    window.makeQueueEntry = function(item, addbtns) {
        ...
        blame.html("Added by: <span class='sbby'>" + item.queueby + "</span>");
        var clear = $("<div/>").addClass("qe_clear").appendTo(li);
        if (item.temp) {
            li.addClass("queue_temp")
        }
        if (addbtns) addQueueButtons(li);
        return li
    };
    setTimeout(function() {
...
document.body.insertAdjacentHTML("beforeEnd", "<style>#queue li .qe_time:before { content: attr(data-timeleft) !important; }</style>");
...
function calculateRemainingTime() {
    ...
    if (q.length == 0) {console.log('test4');  return;}
...
(function() {
    if (!$("#leader").length && CLIENT.rank >= CHANNEL.perms.leaderctl) {
        $("<button>").prop("id", "leader").attr("title", "Control current time of media").addClass("btn btn-sm btn-default").append($("<span>").addClass("glyphicon glyphicon-transfer")).insertAfter($("#shuffleplaylist")).on("click", function() {
            if (CLIENT.leader) {
    ...
    if (!$("#shrinkplaylist").length && CLIENT.rank >= minrank) {
        ...
        $("<button>").prop("id", "shrinkplaylist").attr("title", "Toggle playlist collapse").addClass("btn btn-sm btn-default").append($("<span>").addClass("glyphicon glyphicon-compressed")).insertAfter($("#shuffleplaylist")).on("click", function() {
            if (!$("#queue").data().shrink) {
                ...
                $("head").append($("<style>").prop("id", "playlistStyle").text("#queue div.btn-group, .qe_clear { display: none!important; }"))
            } else {
                $("#queue").data().shrink = false;
...
(function(CyTube_Recent_Media) {
    return CyTube_Recent_Media(window, document, window.jQuery)
})(function(window, document, $, undefined) {
    if (!playlistEnhancer.recentMedia) {
        return
    }
    ...
    if (typeof Storage === "undefined") {
        console.info("[XaeTube: Recent Media] Storage not supported. Aborting load.");
        return
    } else {
        console.info("[XaeTube: Recent Media] Loading Module.");
        if (localStorage[CHANNEL.name + "_RecentMedia"] === undefined) {
            localStorage[CHANNEL.name + "_RecentMedia"] = JSON.stringify([])
        }
    }
    var RecentMedia = window[CHANNEL.name]["RecentMedia"] = {
        maxhist: 10,
        ...
        view: function() {
            function createEntry(media) {
                ...
                var blame = $("<span>").addClass("qe_blame").html("Added by: <span class='sbby'>" + media.blame + "</span>").appendTo(li);
                return li
            }
        ...
        init: function() {
            ...
            var button = $("<button>").attr("id", "showrecent").attr("title", "Recently shown videos").attr("data-toggle", "collapse").attr("data-target", "#recentmedia").addClass("btn btn-sm btn-default collapsed active").append($("<span>").addClass("glyphicon glyphicon-time")).on("click", function() {
                var wasActive = $(this).hasClass("active");
                $(".plcontrol-collapse").collapse("hide");
                $("#plcontrol button.active").button("toggle");
                if (!wasActive) {
                    $('#maincontain .nano-content').stop().animate({
                        scrollTop: $("#maincontain").height()
                    }, "slow");
                    $(this).button("toggle")
                }
                self.view()
            }).insertAfter("#shuffleplaylist").click();
